...
def test_svm_rank_loss_scales_regularization_by_sample_count() -> None:
    config = PairwiseLogisticConfig(
        epochs=1,
        batch_size=2,
        learning_rate=0.1,
    ...
    duplicated_loss = model.loss(duplicated_X, duplicated_y)
    y_signed = np.where(y > 0.5, 1.0, -1.0)
    margins = y_signed * model.predict_pairwise_logits(X)
    mean_squared_hinge = float(np.mean(np.square(np.maximum(0.0, 1.0 - margins))))
    regularization = 0.5 * (
        float(np.sum(model.weights**2))
        + float((model.bias[0] / model.intercept_scaling) ** 2)
    )

    assert np.isclose(base_loss, (regularization / 2.0) + (model.svm_c * mean_squared_hinge))
    assert np.isclose(duplicated_loss, (regularization / 4.0) + (model.svm_c * mean_squared_hinge))
